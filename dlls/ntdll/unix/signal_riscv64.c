...
C_ASSERT( sizeof(struct riscv64_thread_data) <= sizeof(((struct teb_data *)0)->cpu_data) );
C_ASSERT( offsetof( TEB, GdiTebBatch ) + offsetof( struct riscv64_thread_data, exit_frame ) == 0x2f0 );

static inline struct riscv64_thread_data *riscv64_thread_data( struct thread_data *data )
{
    return (struct riscv64_thread_data *)get_teb_data(data)->cpu_data;
}
...
static void setup_exception( ucontext_t *sigcontext, EXCEPTION_RECORD *rec )
{
    ...
    stack = virtual_setup_exception( data, stack_ptr, (sizeof(*stack) + 15) & ~15, rec );
    stack->rec = *rec;
...
{
    get_syscall_frame( data )->x[0] = (UINT64)pKiRaiseUserExceptionDispatcher;
}
...
{
    if (!get_syscall_frame( get_thread_data() )->prev_frame) return STATUS_NO_CALLBACK_ACTIVE;
...
static void usr1_handler( int signal, siginfo_t *siginfo, void *sigcontext )
{
    CONTEXT context;
ERR("here\n");
    if (is_inside_syscall( get_thread_data(), SP_sig((ucontext_t *)sigcontext) ))
    {
        context.ContextFlags = CONTEXT_FULL;
...
static void usr2_handler( int signal, siginfo_t *siginfo, void *sigcontext )
{
    ...
    if (!is_inside_syscall( get_thread_data(), SP_sig(context) )) return;
...
void call_init_thunk( LPTHREAD_START_ROUTINE entry, void *arg, BOOL suspend, TEB *teb )
{
    ...
    *ctx = context;
    ...
    ((void (*)(struct syscall_frame *, NTSTATUS))__wine_syscall_dispatcher_return)( frame, 0 );
}
