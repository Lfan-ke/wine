...
void my_clear_cache(void *begin, void *end)
{
    SIZE_T sz = end - begin;
    NtFlushInstructionCache(GETCURRENTPROCESS, begin, sz);
}
#define __clear_cache my_clear_cache
#endif
...
int Table64(dynarec_native_t *dyn, uint64_t val, int pass)
{
    ...
    //printf_log(LOG_NONE, "delta = %d pass %d val %x\n", delta, pass, val);
    //printf_log(LOG_NONE, "*val %p\n", *(void**)val);
    return delta;
}
...
void* FillBlock64(dynablock_t* block, uintptr_t addr, int alternate, int is32bits) {
    ...
    */
    ...
    if(current_helper) {
        if (BOX64ENV(dynarec_log)) dynarec_log(LOG_DEBUG, "Canceling dynarec FillBlock at %p as another one is going on\n", (void*)addr);
        return (void*)-1;
    }
