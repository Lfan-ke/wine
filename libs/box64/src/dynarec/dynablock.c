...
{
        //printf_log(LOG_DEBUG, "internalDBGetBlock %d\n", __LINE__);
...
#ifndef _WIN32 /* Box64 doesn't know about mappings Hangover already did */
    ...
    if(block) {
        ...
        if (offsetof(x64emu_t, win64_teb) != 3136)
        {
            ...
            while (1)
            {
                dynarec_log(LOG_INFO,  "offsetof(x64emu_t, win64_teb) is unexpectedly %ld\n", offsetof(x64emu_t, win64_teb));
                dynarec_log(LOG_DEBUG, "offsetof(x64emu_t, win64_teb) is unexpectedly %ld\n", offsetof(x64emu_t, win64_teb));
            }
            int *bait2 = (void*)123;
            *bait2 = 32321;
        }
...
dynablock_t* DBGetBlock(x64emu_t* emu, uintptr_t addr, int create, int is32bits)
{
    if(isInHotPage(addr))
        return NULL;
    //printf_log(LOG_DEBUG, "DBGetBlock %d\n", __LINE__);
