    ...
    switch(opcode) {
            ...
            if(PK(0)=='S' && PK(1)=='C') {
                ...
                if(PK64(0)==0)
                {
                    ...
                    *need_epilog = 1;
                    ...
                    tmp = 0;
#endif
                    if((box64_log<2 && !cycle_log && !box64_dynarec_test) && tmp) {
        ...
        case 0xCD:
            u8 = F8;
                ...
                STORE_XEMU_CALL(xRIP);
                MOV32w(x1, u8);
                LDRx_U12(xR8, xEmu, offsetof(x64emu_t, win64_teb)); // xR8 == x18 = win64_tebs
                CALL_S(x86Int, -1);
                LOAD_XEMU_CALL(xRIP);
                TABLE64(x3, addr); // expected return address
                CMPSx_REG(xRIP, x3);
        ...
        case 0xE8:
            ...
            switch(tmp) {
                case 3:
                    ...
                    sse_purge07cache(dyn, ninst, x3);
#if 0 // FIXME (AZ)
                    if((box64_log<2 && !cycle_log && !box64_dynarec_test) && dyn->insts[ninst].natcall) {
